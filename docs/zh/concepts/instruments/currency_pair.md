...
`CurrencyPair` 表示以 `基础货币/计价货币`（`BASE/QUOTE`）形式报价的现货或现金市场。基础货币是被买入或卖出的资产，计价货币则用于为一单位基础货币定价。Nautilus 使用该类型来表示法币外汇对和加密货币现货对。

示例包括 `EUR/USD.SIM`、`BTCUSDT.BINANCE` 以及 `ETH/USD.KRAKEN`。
...
| `base_currency`   | `Currency`         | `Currency`         | 必需             | 被买入或卖出的资产。                       |
| `quote_currency`  | `Currency`         | `Currency`         | 必需             | 用于为基础资产定价的货币。                 |
...
- `CurrencyPair` 的工具类别为 `Spot`（现货）。
- 它没有到期日、行权价、期权类型或衍生品标的字段。
- 它绝不是反向的。结算货币和成本货币均为计价货币。
- 该类型同时适用于法币外汇对和加密货币现货对。

:::warning
不要仅仅因为符号看起来像货币对，就将有到期日的期货、互换或期权建模为 `CurrencyPair`。应使用具体的衍生品类型，以确保成本货币、结算货币、到期日和名义价值计算与交易场所保持一致。
:::
...
- [执行（Execution）](../execution.md) 说明了使用工具精度的订单检查。
