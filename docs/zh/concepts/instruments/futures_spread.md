...
`FuturesSpread` 表示交易所定义的、包含多条腿的期货策略，例如日历价差（calendar spread）或跨商品价差（inter-commodity spread）。交易场所定义该策略、符号、最小报价单位（tick size）和到期日。

示例包括上市的期货日历价差和交易所支持的价差市场。
...
| `underlying`      | `Ustr`             | `str`              | 必需             | 标的产品或产品系列。                        |
...
| `size_precision`  | `u8`               | `int`              | `0`              | 期货价差以整数份合约交易。                  |
...
- `FuturesSpread` 的工具类别为 `FuturesSpread`。
- 交易场所将该价差发布为单一可交易工具。
- 它以整数份合约交易，数量精度为 `0`，数量步长为 `1`。
- 当某策略需要交易场所特定的分腿细节时，可使用适配器元数据中的分腿数据。
...
- [期货合约（Futures Contract）](futures_contract.md) 介绍了单腿期货。
