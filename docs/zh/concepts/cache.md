...
`Cache` 是一个中央内存数据库，存储和管理所有与交易相关的数据，
从市场数据到订单历史，再到自定义计算结果。

Cache 有多重用途：

1. **存储市场数据**：
   - 存储近期的市场历史（例如订单簿、报价、成交、K 线）。
   - 使你的策略能够访问当前和历史市场数据。

2. **跟踪交易数据**：
   - 维护完整的 `Order` 历史和当前执行状态。
   - 跟踪所有 `Position`（仓位）和 `Account`（账户）信息。
   - 存储 `Instrument` 定义和 `Currency`（货币）信息。

3. **存储自定义数据**：
   - 你可以将任何用户定义的对象或数据存入 `Cache` 供以后使用。
   - 实现不同策略之间的数据共享。
...
- 系统会在数据流经系统时自动将其添加到 `Cache` 中。
- 在实盘环境中，引擎异步应用更新，因此在事件发生和它出现在 `Cache` 中之间可能存在短暂的延迟。
- 对于报价、成交和 K 线，`DataEngine` 会先写入 `Cache`，然后再发布给订阅者，因此当你的处理器运行时，缓存中已经有了最新值。订单簿差量和深度快照会被直接发布，不会写入缓存；订单簿状态是通过 `BookUpdater` 订阅单独维护的：
...
有关完整的逐步追踪过程，请参阅
[数据流转：一个报价 tick 的生命周期](architecture.md#data-flow-life-of-a-quote-tick)。

### 基础示例

在策略内部，你可以通过 `self.cache` 访问 `Cache`。这是一个典型的示例：
...
根据你的[环境上下文](architecture.md#environment-contexts)，你可以将该配置提供给
`BacktestEngine` 或 `TradingNode`。
...
:::tip
默认情况下，`Cache` 会为每种 K 线类型保留最近 10,000 根 K 线，
并为每个金融工具保留最近 10,000 条成交 tick。这些限制在内存使用和
数据可用性之间提供了良好的平衡。如果你的策略需要更多历史数据，
可以增大这些限制。
:::
...
:::note
每种 K 线类型都维护各自独立的容量。例如，如果你同时使用 1 分钟和
5 分钟 K 线，每种都会存储最多 `bar_capacity` 根 K 线。
当达到 `bar_capacity` 时，`Cache` 会自动移除最旧的数据。
:::

### 数据库配置

为了在系统重启之间实现持久化，你可以配置一个数据库后端。
`CacheConfig` 只控制缓存的行为。连接设置属于具体的缓存
数据库技术配置，例如 `RedisCacheConfig` 或 `PostgresCacheConfig`。

什么时候使用持久化会有帮助？

- **长期运行的系统**：如果你希望数据在系统重启、升级或意外故障后依然存在，数据库配置能帮助你准确地从上次中断的地方继续。
- **历史洞察**：当你需要保留过去的交易数据以进行详细的事后分析或审计时。
- **多节点或分布式部署**：如果多个服务或节点需要访问同一份
  状态，持久化存储有助于确保数据的共享和一致性。

Rust 原生调用方需要构建一个具体的数据库配置，并使用
`CacheDatabaseFactory` trait 来构造传入系统构建器的适配器：
...
在默认的 `LiveExecEngineConfig.load_cache = true` 下，节点会在连接客户端或
核对执行状态之前恢复持久化的缓存状态，并重建派生索引。设置
`CacheConfig.flush_on_start = true` 会改为清空后端存储。目前 Python v2
的 `LiveNode` 接口尚不支持直接注入后端存储。

## 使用缓存

### 访问市场数据

`Cache` 提供了访问订单簿、报价、成交和 K 线的完整接口。
缓存中的所有市场数据都使用反向索引，因此最新的条目位于索引 0 处。
...
你可以通过多种方法访问和查询订单，并可按交易场所、策略、金融工具
和订单方向灵活过滤。
...
长期运行的会话会积累已关闭的订单、已关闭的仓位、账户事件以及
未使用的金融工具。缓存暴露了有针对性的和批量的清理方法，
使策略和实盘交易引擎能够在不重启系统的情况下控制内存占用。

#### 有针对性的清理

使用这些方法删除单个实体。只要该实体仍处于活跃状态，每种方法都会拒绝清理。

- `cache.purge_order(client_order_id)`：移除该订单以及每一个按订单键控的
  索引条目。会跳过未平仓订单。
- `cache.purge_position(position_id)`：移除该仓位、其快照以及按仓位键控的
  索引条目。会跳过未平仓仓位。
- `cache.purge_instrument(instrument_id)`：移除该金融工具以及每一个
  按金融工具的映射（订单簿、报价、成交、标记/指数/资金费率价格、
  金融工具状态、希腊值，以及引用该金融工具的 K 线）。只要与之关联的
  任何订单处于非终止状态（任何尚未达到已关闭状态的订单，包括已初始化、
  已提交、已接受、模拟中、已释放和在途订单），或任何关联仓位处于
  非已关闭状态，就会跳过清理。
...
:::warning
`purge_instrument` 适用于拥有自己生命周期逻辑、能够决定何时不再需要
某个金融工具的 actor 和策略。清理一个其他组件仍然依赖的金融工具，
会导致金融工具查找缺失并丢失市场数据历史。活跃订阅归数据引擎所有，
如果你不再需要相关更新，请在清理之前先取消订阅。
:::

#### 批量清理

使用这些方法按时效清扫较旧的条目。它们接受当前时间戳和以秒为单位的
缓冲期或回溯窗口。

- `cache.purge_closed_orders(ts_now, buffer_secs)`：关闭时间戳早于
  `buffer_secs` 的已关闭订单。
- `cache.purge_closed_positions(ts_now, buffer_secs)`：关闭时间戳早于
  `buffer_secs` 的已关闭仓位。
- `cache.purge_account_events(ts_now, lookback_secs)`：早于
  `lookback_secs` 的账户状态事件。值为 `0` 时会清理所有事件。

#### 实盘交易中的自动清理

`LiveExecEngineConfig` 会将批量清理调度到一个定时器上。设置该间隔
即可启用该循环，并通过缓冲期或回溯期来控制多近的条目会被保护。
以下默认值适用于大多数实盘会话：
...
60 分钟的缓冲期能够保留最近的活动记录用于核对，同时依然能够
削减长尾数据的增长。对于高频交易（HFT）会话可以调低这些值，
如果分析需要更长的历史回溯期则可以调高。完整的参数参考请参阅
[配置实盘交易：内存管理](../how_to/configure_live_trading.md)。

:::note
金融工具的清理没有自动循环，因为何时清理某个金融工具取决于
策略状态，而不是时效。请从拥有该金融工具生命周期的 actor 或
策略中调用 `cache.purge_instrument`。
:::
...
除了内置的市场数据和交易对象之外，`Cache` 还可以存储和检索
自定义数据类型。可用它在系统组件（主要是 actor 和策略）之间
共享任何用户定义的数据。
...
对于更复杂的使用场景，`Cache` 可以存储继承自
`nautilus_trader.core.Data` 基类的自定义数据对象。

:::warning
`Cache` 并非设计为完整数据库的替代品。对于大型数据集或复杂的
查询需求，请考虑使用专门的数据库系统。
:::

## 最佳实践与常见问题

### Cache 与 Portfolio 的使用对比

`Cache` 和 `Portfolio` 组件在 NautilusTrader 中扮演着不同但互补的角色：

**Cache**：

- 维护交易系统的历史知识和当前状态。
- 在本地状态发生变化时立即更新（例如，提交前初始化一个订单）。
- 随着外部事件的发生异步更新（例如，某个订单成交时）。
- 提供交易活动和市场数据的完整历史。
- 保留策略收到的每一个事件。
...
在将数据存储于 `Cache` 还是策略变量之间做选择，取决于你的具体需求：
...
以下示例展示了如何将数据存储在 `Cache` 中，以便多个策略可以访问
相同的信息。
